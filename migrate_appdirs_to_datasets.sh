...
#
# Safety model:
# - Stops Docker first (containers AND daemon), verifies both are down.
...
# sha256sum -z requires coreutils >= 8.31; check for support
...
docker_img_is_mounted() {
  mount 2>/dev/null | grep -q 'docker.img'
}
...
stop_docker() {
  if docker_daemon_is_running; then
    ...
    if containers_are_running; then
      log "Some containers still running; trying docker kill..."
      mapfile -t running_ids < <(docker ps -q 2>/dev/null) || true
      if [[ ${#running_ids[@]} -gt 0 ]]; then
        docker kill "${running_ids[@]}" 2>/dev/null || true
      fi
      sleep 5
    fi

    if containers_are_running; then
      die "Containers still running after stop and kill. Aborting."
    fi

    log "All containers stopped. Stopping Docker daemon..."
    /etc/rc.d/rc.docker stop || true
    sleep 10

    if docker_daemon_is_running; then
      log "Docker daemon still running; sending SIGKILL to dockerd..."
      pkill -9 -x dockerd 2>/dev/null || true
      sleep 5
    fi

    if docker_daemon_is_running; then
      die "Docker daemon did not stop. Aborting."
    fi

    if docker_img_is_mounted; then
      die "docker.img loopback is still mounted after Docker stop. Aborting."
    fi

    log "Docker is fully stopped (was running: yes)."
  else
    log "Docker daemon already stopped (was running: no)."
    if containers_are_running; then
...
get_directory_used_bytes() {
  local dir="$1"
  # -sk gives 1 KiB blocks; multiply to get bytes.
  # Uses -x to stay on one filesystem.
